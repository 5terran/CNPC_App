...
namespace CNPC_App.Ali
{
    public class OssHelper
    {
        ...
        public static string PutObject(Stream fs, string fileName)
        {
            ...
            string key = fileName;//DateTime.Now.Date.ToString("yyyyMMdd") + "/" + fileName;
            ...
            using (fs)
            {
                ...
                PutObjectResult pobjResult = client.PutObject(bucketName, key, fs, metadata);

                if (pobjResult.ETag != null && pobjResult.ETag.Length > 0)
                    return pobjResult.ETag + "上传成功";
